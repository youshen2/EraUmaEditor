...
public class AbilityFragment extends Fragment {
    ...
    private void initEditorItems() {
        try {
            JSONObject item_hold = EraUmaEditor.save_data.getJSONObject("abl").getJSONObject(String.valueOf(EraUmaEditor.current_character_id));

            Iterator<String> keys = item_hold.keys();
            while (keys.hasNext()) {
                String key = keys.next();

                items.add(new EditorItem.Builder(Constants.ABL_MAP.get(key), "abl/{id}/" + key, EditorItem.DataType.INT)
                        .setLayoutType(EditorItem.LayoutType.DOUBLE)
                        .setSuffix("级")
                        .build());

                if (Integer.parseInt(key) == 7) items.add(EditorItem.createHeader("性爱等级"));
            }
        }catch (Exception e){
            e.printStackTrace();
        }
