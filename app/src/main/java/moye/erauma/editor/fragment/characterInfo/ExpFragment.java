...
public class ExpFragment extends Fragment {
    ...
    private void initEditorItems() {
        try {
            JSONObject item_hold = EraUmaEditor.save_data.getJSONObject("exp").getJSONObject(String.valueOf(EraUmaEditor.current_character_id));

            Iterator<String> keys = item_hold.keys();
            while (keys.hasNext()) {
                String key = keys.next();

                items.add(new EditorItem.Builder(Constants.EXP_MAP.get(key), "exp/{id}/" + key, EditorItem.DataType.INT)
                        .setLayoutType(EditorItem.LayoutType.DOUBLE)
                        .build());

                if (Integer.parseInt(key) == 20) items.add(EditorItem.createHeader("爱慕相关"));
            }
        }catch (Exception e){
            e.printStackTrace();
        }
