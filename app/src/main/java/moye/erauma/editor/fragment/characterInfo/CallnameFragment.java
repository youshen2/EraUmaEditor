...
public class CallnameFragment extends Fragment {
    ...
    private void initEditorItems() {
        try {
            JSONObject item_hold = EraUmaEditor.save_data.getJSONObject("callname").getJSONObject(String.valueOf(EraUmaEditor.current_character_id));

            Iterator<String> keys = item_hold.keys();
            while (keys.hasNext()) {
                String key = keys.next();

                items.add(new EditorItem.Builder("对 " + EraUmaEditor.save_data.getJSONObject("callname").getJSONObject(key).getString("-1") + " 的称呼", "callname/{id}/" + key, EditorItem.DataType.STRING)
                        .build());
            }
        }catch (Exception e){
            e.printStackTrace();
