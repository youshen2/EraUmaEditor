...
public class ItemFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ((CollapsingToolbarLayout) view.findViewById(R.id.collapsing_toolbar)).setTitle(getResources().getString(R.string.title_item));
    ...
    private void initEditorItems() {
        try {
            JSONObject item_hold = EraUmaEditor.save_data.getJSONObject("item").getJSONObject("hold");

            Iterator<String> keys = item_hold.keys();
            while (keys.hasNext()) {
                String key = keys.next();

                items.add(new EditorItem.Builder(Constants.ITEM_MAP.get(key), "item/hold/" + key, EditorItem.DataType.INT)
